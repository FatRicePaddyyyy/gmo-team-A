#!/usr/bin/env bash
# check-e2e-kitaqsign.sh
# POST /api/v1/public/domains/check の網羅検証 (Kitaqsign = .com)。
#
# 使い方:
#   ./scripts/lifecycle/check-e2e-kitaqsign.sh --env .env
#
# 前提: backend が localhost:8787 で起動、Kitaqsign が生きていること。
#
# 検証項目:
#   (a) 空きあり: 存在しなさそうな .com  → 200 avail=true
#   (b) 空きなし: 予約されている .com    → 200 avail=false or reason 付き
#   (c) 不正形式: TLD 無し              → 400
#   (d) 非対応TLD: kitaqsign が持たない  → 400
...
# avail は true / false どちらでも構わない (レジストリの登録状況次第) が、200 で返ること自体を確認
...
if [ "${STATUS_D}" = "400" ] || [ "${STATUS_D}" = "422" ] || [ "${STATUS_D}" = "500" ]; then
  # 500 は片側 hello が失敗したときの network_error 経路。レジストリ疎通異常時のため許容。
