#!/usr/bin/env bash
# create-e2e-kitaqnic.sh
# POST /api/v1/secure/domains の網羅検証 (Kitaqnic = .xyz)。
#
# 使い方:
#   ./scripts/lifecycle/create-e2e-kitaqnic.sh --env .env
...
if echo "${BODY_A}" | grep -q '"registry":"kitaqnic"'; then
  ok "registry=kitaqnic と判定"
else
  ng "registry=kitaqnic と判定されていない: ${BODY_A}"
fi
