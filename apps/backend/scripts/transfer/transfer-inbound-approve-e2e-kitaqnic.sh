...
#   4. teama: backend API で approve
#   5. teama: /secure/domains にそのドメインが無いことを確認
#   6. teama-2: /api/v1/epp/domains/{name} でドメインが見えることを確認
#
# 前提:
...
# ─────────────────────────────────────────────
# 4. teama backend で approve
# ─────────────────────────────────────────────
step "[4] teama: backend API で approve"
RAW="$(teama_call POST "/api/v1/secure/domains/${DOMAIN_ID}/transfer/approve")"
ST="$(status_of "$RAW")"; BD="$(body_of "$RAW")"
info "  → HTTP:${ST}"
[ "$ST" = "200" ] || fail "[4] backend approve 失敗 HTTP:${ST} body=$(printf '%s' "$BD" | head -c 300)"
ok "[4] backend approve 成功"

# ─────────────────────────────────────────────
# 5. teama backend からドメインが消えていることを確認
# ─────────────────────────────────────────────
step "[5] teama: /secure/domains に該当ドメインが無いことを確認"
RAW="$(teama_call GET /api/v1/secure/domains)"
ST="$(status_of "$RAW")"; BD="$(body_of "$RAW")"
[ "$ST" = "200" ] || fail "[5] /secure/domains 取得失敗 HTTP:${ST}"
if echo "$BD" | grep -q "\"${DOMAIN}\""; then
  fail "[5] teama backend にドメイン ${DOMAIN} が残っている: $(echo "$BD" | head -c 400)"
fi
# DB からも消えてる確認
DB_ROW="$(sqlite3 "${D1_SQLITE}" "SELECT id FROM domains WHERE name='${DOMAIN}';")"
[ -z "$DB_ROW" ] || fail "[5] DB の domains 行が残っている: ${DB_ROW}"
ok "[5] teama backend からドメイン消滅を確認 (API + DB 両方)"

# ─────────────────────────────────────────────
# 6. teama-2 レジストリ側でドメインが見えることを確認
# ─────────────────────────────────────────────
step "[6] teama-2: registry でドメインが見えることを確認"
...
ok "[6] teama-2 でドメインが見える (承認完了)"

# ─────────────────────────────────────────────
# 総括
# ─────────────────────────────────────────────
printf "\n${GREEN}=== 6 ステップ e2e 完了 ===${RESET}\n"
printf "  domain     : %s\n" "$DOMAIN"
printf "  domain_id  : %s\n" "$DOMAIN_ID"
printf "  authInfo   : %s\n" "$AUTH_INFO"
printf "  losing     : teama\n"
printf "  gaining    : teama-2\n"
