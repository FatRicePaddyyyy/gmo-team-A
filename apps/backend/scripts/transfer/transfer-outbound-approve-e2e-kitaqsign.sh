#!/usr/bin/env bash
# transfer-outbound-approve-e2e-kitaqnic.sh (kitaqsign / .com)
# 6 ステップ e2e (teama-2 losing / teama gaining, teama は backend 経由):
#   1. teama-2: registry でドメイン作成 (authInfo 取得)
#   2. teama:   backend API で移管申請 (POST /api/v1/secure/transfers, authInfo 付き)
#              → backend は outbound_transfer_requests に INSERT + registry.transferRequest
#   3. teama-2: registry で poll → op=request メッセージが来る (script が ack)
#   4. teama-2: registry で transfer/approve
#   5. teama-2: registry で info 確認 (承認後の状態確認)
#   6. teama:   /__scheduled で cron 発火 → backend が承認通知を検知して domain 行 INSERT
#              → teama の /api/v1/secure/domains に該当ドメインが出現することを確認
#
# 前提:
...
# kitaqsign は POST /messages/{id}/ack で ack (kitaqnic は DELETE)
ack_registry_msg() {
  local id="$1"
  t2_call POST "/api/v1/epp/messages/${id}/ack"
}
...
sleep 1
# キューに残骸メッセージがあれば ack で drain して該当ドメインに辿り着く。
FOUND=0
for i in $(seq 1 30); do
  POLL_RAW="$(t2_call GET /api/v1/epp/messages/poll)"
  ...
  [ -z "$MSG_ID" ] && { warn "  キューが空 (該当メッセージ未着)"; break; }
...
[ "$FOUND" = "1" ] || fail "[3] 該当 op=request メッセージが見つからなかった"
ok "[3] teama-2 で op=request 受信・ack 完了"

# ─── 4. teama-2 registry で approve ──────────────────────────
step "[4] teama-2: registry で transfer/approve"
RAW="$(t2_call POST "/api/v1/epp/domains/${DOMAIN}/transfer/approve")"
ST="$(status_of "$RAW")"; BD="$(body_of "$RAW")"
[ "$ST" = "200" ] || fail "[4] teama-2 approve 失敗 HTTP:${ST} body=$(printf '%s' "$BD" | head -c 300)"
APP_STATUS="$(json_get "$BD" 'resData.status')"
APP_GAIN="$(json_get "$BD" 'resData.gainingRegistrar')"
APP_LOSE="$(json_get "$BD" 'resData.losingRegistrar')"
info "  status=${APP_STATUS} gaining=${APP_GAIN} losing=${APP_LOSE}"
[ "$APP_GAIN" = "${TEAMA_REG}" ] || fail "[4] gainingRegistrar が teama でない (${APP_GAIN})"
ok "[4] teama-2 approve 成功 (所有権 teama-2 → teama)"

# ─── 5. teama-2 registry で info 確認 ────────────────────────
step "[5] teama-2: registry で info 確認"
...
ok "[5] teama-2 registry から info 200 (承認完了)"

# ─── 6. teama backend で cron 発火 → domain 出現確認 ─────────
step "[6] teama: /__scheduled で cron 発火 → backend DB に domain 行が INSERT される"
trigger_cron
info "  cron 発火完了、DB 検証..."

# DB に domain 行が出現したか確認
DB_DOM="$(sqlite3 "${D1_SQLITE}" "SELECT id||'|'||status||'|'||owner_user_id FROM domains WHERE name='${DOMAIN}';")"
[ -n "$DB_DOM" ] || fail "[6] backend DB に domain 行が無い"
info "  DB domain row = ${DB_DOM}"

# outbound_transfer_requests が clientApproved / serverApproved に更新されているか
...
case "${DB_OUT_NEW}" in
  clientApproved|serverApproved) ok "  outbound が承認確定" ;;
  *) fail "[6] outbound が承認確定していない (${DB_OUT_NEW})" ;;
esac

# teama backend API 経由でも domain が見えるか
RAW="$(teama_call GET /api/v1/secure/domains)"
ST="$(status_of "$RAW")"; BD="$(body_of "$RAW")"
[ "$ST" = "200" ] || fail "[6] /secure/domains 取得失敗 HTTP:${ST}"
if ! echo "$BD" | grep -q "\"${DOMAIN}\""; then
  fail "[6] teama backend の /secure/domains に ${DOMAIN} が出てこない: $(echo "$BD" | head -c 400)"
fi
ok "[6] teama backend に domain 出現確認 (DB + API 両方)"

# ─── 総括 ────────────────────────────────────────────────────
printf "\n${GREEN}=== 6 ステップ e2e (outbound: teama-2 → teama) 完了 ===${RESET}\n"
printf "  domain     : %s (teama 所有に確定)\n" "$DOMAIN"
