...
export function TransferRequestForm({
  ...
  const onSubmit = async (data: TransferRequestFormData) => {
    if (ownedNames.includes(data.name)) {
      setError("name", {
        message:
          // 「下の〜から進んでください」と書いていたが、その案内は同じ画面の上（見出しの下）にあり、
          // しかもリンクではなく説明文なので、指し示す先として成立していなかった。
          // 実際にたどる手順をそのまま書く。
          "このドメインはすでにここにあるので、引き取る必要はありません。他のレジストラへ渡したい場合は、マイドメインから対象のドメインを開き、設定の中で手続きしてください。",
      });
      return;
    }
