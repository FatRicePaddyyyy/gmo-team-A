...
/**
 * @registry-kitaqnic-outage — 移管 outbound cancel (.xyz) が始められないこと。
 */
test.describe(
  "kitaqnic 障害時: outbound cancel シナリオが始められない",
  { tag: "@registry-kitaqnic-outage" },
  () => {
    test.skip(!hasSeedEnv(), "SECRET_KEY が無いためスキップ");

    test(".xyz 申請で「申請中の移管」に載らない", async ({ page }) => {
      const user = await createSeedUser({ label: "tr-out-c-outage" });
      await loginAndExpectDashboard(page, user);

      const fullDomain = `tr-nic-out-c-${Date.now()}${Math.floor(Math.random() * 1_000_000)
        .toString()
        .padStart(6, "0")}.xyz`;
